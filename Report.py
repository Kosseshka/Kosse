...
class Statistics:
    ...
    def filtering(self, vacancy: dict):
        ...
        salary = int(mean((float(vacancy['salary_from']), float(vacancy['salary_to']))) * currency_to_rub[
    ...
    def print_vacancies(self):
        ...
        """
        salaryYear = dict(
            sorted({key: int(mean(value)) for key, value in self.salaryYear.items()}.items(), key=lambda item: item[0]))
        numberVacancies = dict(sorted(self.numberVacancies.items(), key=lambda item: item[0]))
        selectedSalaryYear = dict(sorted({key: (int(mean(value)) if len(value) > 0 else 0) for key, value in
                                          self.selectedSalaryYear.items()}.items(), key=lambda item: item[0]))
        selectedNumberVacancies = dict(sorted(self.selectedNumberVacancies.items(), key=lambda item: item[0]))
        salaryCity = dict(
            sorted({key: int(mean(value)) for key, value in self.salaryCity.items()}.items(), key=lambda item: item[1],
                   reverse=True)[0:10])
        vacanciesCity = dict(
            sorted({key: round(value / self.counts, 4) for key, value in self.vacanciesCity.items()}.items(),
...
class Report:
    ...
    def generate_image(self, dataList: dict):
        ...
        vacanciesCity = dataList['vacanciesCity'].values() if others == 0 else [others] + list(
            dataList['vacanciesCity'].values())
